...
A supply chain analytics project built on the [DataCo Supply Chain dataset](https://www.kaggle.com/datasets/shashwatwork/dataco-smart-supply-chain-for-big-data-analysis) (~180K order records), combining SQL analysis with a machine learning model to flag late-delivery risk before it happens.
...
- **matplotlib** — visualizations of delivery status, late rate by region, and late rate by shipping mode
...
A dashboard built on top of this analysis, so the findings are explorable interactively rather than only through scripts and static charts.
